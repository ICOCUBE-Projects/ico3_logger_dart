...
void main() {
  ...
  Log.setCategories('<clear> core, ui(critical)');  // select log on categories and level
  Log.warning('core', 'Waiting core initialisation');   //selected
  Log.warning('ui', 'try to restart initialisation');    //not selected lower to critical
  Log.critical('ui', 'cannot restart initialisation');    // selected

  // ADD TIMESTAMP
  Log.setDecoration(category: true, timeStamp: true);  // Add more Log Information
  ...
  Log.saveMessageList(logFileName: 'log1.csv', format: SaveFormat.csv); //output to File
  Log.processMessageList(onLogMessage: (message) { // process messages list in custom function
    print(message.message);
  });
  for (var message in Log.getMessageList()) { // use directly the messages stored
    print('--> ${message.message}');
  }
...
// SET DECORATION TO PUT CONSOLE LOG MESSAGE
  //----------------------------------------------------------
  Log.setDecoration(
      timeStamp: true, loggerID: true, category: true, mode: 'emoji');    // Add emoji
  Log.info('app', 'App function A');
  Log.critical('network', 'connection critical');

  // use custom emoji
  Log.setDecoration(      // with custom emoji
      timeStamp: true, loggerID: true, category: true, mode: 'emoji', emoji: '😛,🙂,😨,🥶,🥵,🤬');
  Log.info('app', 'App function A');
  Log.critical('network', 'connection critical');

  Log.setDecoration(      // Level colored depending level
      timeStamp: true,
      loggerID: true,
      category: true,
      mode: 'level',
